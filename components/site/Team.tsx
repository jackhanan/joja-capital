...
export default function Team({ team }: { team: TeamContent }) {
  return (
    <section id="team" className="border-b border-slate-800/60">
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {team.items.map((member) => (
            <div key={member.id} className="text-left">
              ...
              />
              <h3 className="mt-5 font-serif text-lg text-slate-50">
                {member.name}
              </h3>
              <p className="mt-1 text-gold-400 text-xs uppercase tracking-[0.15em]">
                {member.title}
              </p>
