  ...
  return (
    <section id="services" className="border-b border-slate-200">
      <div className="max-w-content mx-auto px-6 sm:px-10 py-24 sm:py-32">
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-200 border border-slate-200">
          {services.items.map((item) => (
            <div
              key={item.id}
              className="bg-white p-8 sm:p-10 hover:bg-slate-50 transition-colors duration-200"
            >
              <div className="font-serif text-accent-600 text-2xl mb-6">
                {item.number}
              </div>
